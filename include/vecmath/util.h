...
namespace vm {
    ...
    constexpr std::array<T, S> cast_array(const std::array<U, S>& a) {
        return detail::cast_array_helper<T>(a, std::make_index_sequence<S>{});
    }
    ...
    constexpr std::array<vec<T, R>, C> cast_matrix_rows(const std::array<vec<U, R>, C>& a) {
        return detail::cast_matrix_rows_helper<T>(a, std::make_index_sequence<C>{});
    }
    ...
    constexpr std::array<T, S1+S2> concat(
        ...
        return detail::concat_helper(a1, std::make_index_sequence<S1>{}, a2, std::make_index_sequence<S2>{});
    }
    ...
    constexpr std::array<T, S> to_array(std::initializer_list<T> v) {
        return detail::to_array_helper<T, S>(v, std::make_index_sequence<S>());
    }
    ...
    constexpr std::array<vec<T, R>, C> to_vec_array(std::initializer_list<T> v) {
        return detail::to_vec_array_helper<T, R, C>(v, std::make_index_sequence<C>());
    }
    ...
    constexpr std::array<std::size_t, S> sort(const std::array<T, S>& a, const C& cmp) {
        ...
        detail::sort_helper(a, s, 0, S, cmp);
        return s;
    }
}
