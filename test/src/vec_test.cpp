...
namespace vm {
    ...
    TEST(vec_test, is_parallel) {
        constexpr auto result = is_parallel(vec3f::zero(), vec3f::zero());
        ASSERT_FALSE(result);

        ASSERT_FALSE(is_parallel(vec3f::zero(), vec3f::zero()));
    ...
    TEST(vec_test, parseValidString) {
        const std::string s("1.0 3 3.5");
        ASSERT_TRUE(vec3f::canParse(s));
        ASSERT_EQ(vec3f(1.0f, 3.0f, 3.5f), vec3f::parse(s));
    }

    TEST(vec_test, parseShortString) {
        const std::string s("1.0 3");
        ASSERT_FALSE(vec3f::canParse(s));
        ASSERT_EQ(vec3f::zero(), vec3f::parse(s));
    }

    TEST(vec_test, parseLongString) {
        const std::string s("1.0 3 4 5");
        ASSERT_TRUE(vec3f::canParse(s));
        ASSERT_EQ(vec3f(1.0f, 3.0f, 4.0f), vec3f::parse(s));
    }

    TEST(vec_test, parseInvalidString) {
        const std::string s("asdf");
        ASSERT_FALSE(vec3f::canParse(s));
        ASSERT_EQ(vec3f::zero(), vec3f::parse(s));
    }

    TEST(vec_test, parseEmptyString) {
        const std::string s("");
        ASSERT_FALSE(vec3f::canParse(s));
        ASSERT_EQ(vec3f::zero(), vec3f::parse(s));
    }

    TEST(vec_test, parseAll) {
